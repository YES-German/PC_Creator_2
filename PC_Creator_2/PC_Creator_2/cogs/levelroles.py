...
class levelroles(commands.Cog):
    ...
    async def lead(self, ctx):
        await self.lrs_stats()
        with open("userLevels.json", "r") as f:
            data = json.load(f)
            f = discord.File("C:\\Users\\ja\\Desktop\\Neuer Ordner\\PC_Creator_2\\dailymsgs.png", filename="dailymsgs.png")
    ...
    async def lrs_stats(self):
        ...
        myFont = ImageFont.truetype(r'CALIBRI.TTF', 20)
        x = 0
        y = 500/hunderter
        for i in range(hunderter):
            y_1 = y*x+40
    ...
    schedule.every().day.at("00:00").do(lrs_stats)       
